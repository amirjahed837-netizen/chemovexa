...
export function MolarMass() {
  const { t } = useI18n();
  const m = t.pages.calculator.molar;
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-5">
      <div className="space-y-5 lg:col-span-3">
        <Field label={m.field} hint={m.hint}>
          <TextInput
            value={input}
            onChange={setInput}
          ...
          <div className="space-y-2">
            <div className="flex items-baseline justify-between rounded-xl border border-cyan-400/30 bg-cyan-400/[0.07] px-5 py-4">
              <span className="text-sm text-cyan-100">
                M<sub>r</sub>{" "}
                <span className="ltr-embed font-mono font-semibold">{prettyFormula(input.trim())}</span>
              </span>
            ...
            />
            <MethodNote>{m.method}</MethodNote>
          </div>
        )}
      </div>
